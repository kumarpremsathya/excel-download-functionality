...
def export_to_excel(request, data, date_range, start_date, end_date, source_name):
    ...
    response['Content-Disposition'] = 'attachment; filename=rbi_log_data.xlsx'

    workbook = Workbook()
    worksheet = workbook.active

    header_font = Font(bold=True)
    headers = ['Source Name', 'Script Status', 'Failure Reason', 'Data Available', 'Data Scraped', 'Date of Scraping', 'Status Color']
    ...
    for row_num, data_entry in enumerate(excel_data, start=2):
        ...
        worksheet.cell(row=row_num, column=3, value=data_entry.failure_reason)
        worksheet.cell(row=row_num, column=4, value=data_entry.data_available)
        worksheet.cell(row=row_num, column=5, value=data_entry.data_scraped)
